...
function RojmelReg() {
  ...
  const getDiamondList = () => {
    axios
      .get(`${process.env.REACT_APP_API}/rojmel?page=${page}&per_page=1000`)
      .then((res) => {
  ...
  const handleSubmitDiamond = (diamond_data) => {
    let final_data = diamond_data.map((data, index)=>{
      return(
        {
          cr_amount : _.get(data,"CR. amt",""),
          cr_vigat: _.get(data,"CR. vigat",""),
          cr_date: _.get(data,"CR. date",""),
          dr_amount: _.get(data,"DR. amt",""),
          dr_vigat: _.get(data,"DR. vigat",""),
          dr_date: _.get(data,"DR. date",""),
        }
      )
    })
    ...
    fetch(`${process.env.REACT_APP_API}/rojmel`, {
      method: "POST",
  ...
  const columns = [
    { id: "cr_amount", name: "CR. amt" },
    { id: "cr_vigat", name: "CR. vigat" },
    { id: "cr_date", name: "CR. date" },
    { id: "dr_amount", name: "DR. amt" },
    { id: "dr_vigat", name: "DR. vigat" },
    { id: "dr_date", name: "DR. date" },
